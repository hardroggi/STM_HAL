...
const uint8_t Test_Plaintext[PLAINTEXT_LENGTH] =
  {
...
uint8_t Test_Key[CRL_AES128_KEY] =
  {
...
const uint8_t Test_Expected_Ciphertext[PLAINTEXT_LENGTH] =
  {
...
int32_t AES_CBC_Decrypt_Append (AESCBCctx_stt *P_pAESCBCctx, const uint8_t *P_pInputBuffer, int32_t P_inputSize, uint8_t *P_pOutputBuffer, int32_t *P_pOutputSize){
	for(int i=0; i<PLAINTEXT_LENGTH; i++){
		P_pOutputBuffer[i] = Test_Plaintext[i];
	}
...
int ut_Test_regular_decryption(){
    ...
    Crypto_AES newAES;
    CHECK(0 == newAES.Crypto_AES_intialize(Test_Key, sizeof(Test_Key), Test_IV, sizeof(Test_IV)));
    CHECK(0 == newAES.Crypto_AES_decrypt((uint8_t *)Test_Expected_Ciphertext, sizeof(Test_Expected_Ciphertext), OutputMessage, &OutputMessageLength));
	for(uint32_t i=0; i<OutputMessageLength; i++){
		CHECK(OutputMessage[i] == Test_Plaintext[i]);
	}
    CHECK(0 == newAES.Crypto_AES_close_decryption_process(OutputMessage, &OutputMessageLength));
...
int ut_Test_initialize_invalid_inputs(){
    ...
    Crypto_AES newAES;
	CHECK(1 == newAES.Crypto_AES_intialize(Test_Key, sizeof(Test_Key)-1, Test_IV, sizeof(Test_IV))); //invalid Input of Keysize
	CHECK(2 == newAES.Crypto_AES_intialize(Test_Key, sizeof(Test_Key), Test_IV, sizeof(Test_IV)+1)); //invalid Input of IVsize

    CHECK(0 == newAES.Crypto_AES_intialize(Test_Key, sizeof(Test_Key), Test_IV, sizeof(Test_IV)));
    CHECK(0 == newAES.Crypto_AES_decrypt((uint8_t *)Test_Expected_Ciphertext, sizeof(Test_Expected_Ciphertext), OutputMessage, &OutputMessageLength));
    CHECK(3 == newAES.Crypto_AES_intialize(Test_Key, sizeof(Test_Key), Test_IV, sizeof(Test_IV))); 	//Setting a new key after starting the decryption
...
int ut_Test_decrypt_invalid_inputs()
{
    ...
    Crypto_AES newAES;
    CHECK(1 == newAES.Crypto_AES_decrypt((uint8_t *)Test_Expected_Ciphertext, sizeof(Test_Expected_Ciphertext), OutputMessage, &OutputMessageLength)); //Not Initialized

    CHECK(0 == newAES.Crypto_AES_intialize(Test_Key, sizeof(Test_Key), Test_IV, sizeof(Test_IV)));
    CHECK(4 == newAES.Crypto_AES_decrypt((uint8_t *)Test_Expected_Ciphertext, sizeof(Test_Expected_Ciphertext)-1, OutputMessage, &OutputMessageLength));
...
int ut_Test_close_invalid_inputs(){
    ...
    Crypto_AES newAES;
    CHECK(2 == newAES.Crypto_AES_close_decryption_process(OutputMessage, &OutputMessageLength));
    CHECK(0 == newAES.Crypto_AES_intialize(Test_Key, sizeof(Test_Key), Test_IV, sizeof(Test_IV)));
    CHECK(2 == newAES.Crypto_AES_close_decryption_process(OutputMessage, &OutputMessageLength));
    CHECK(0 == newAES.Crypto_AES_decrypt((uint8_t *)Test_Expected_Ciphertext, sizeof(Test_Expected_Ciphertext), OutputMessage, &OutputMessageLength));
