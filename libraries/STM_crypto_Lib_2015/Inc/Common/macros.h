 ...
 * @retval  none
 */
#define WORD32_TO_WORD8(P_x, P_a, P_b, P_c, \
                        P_d) P_a = (uint8_t)BYTE_3(P_x), P_b = (uint8_t)BYTE_2(P_x), P_c = (uint8_t)BYTE_1(P_x), P_d = \
    (uint8_t)BYTE_0(P_x)
 ...
 */
#define WORD8_TO_WORD32(P_b0, P_b1, P_b2, P_b3) (uint32_t)((uint32_t)(P_b0) << 24 | (uint32_t)(P_b1) << 16 | \
                                                           (uint32_t)(P_b2) << 8 | (P_b3))
...
#else
#define LE_CONVERT_W32(VariableToConvert) (uint32_t)((ROL((uint32_t)VariableToConvert, \
                                                          24) & \
                                                      0xFF00FF00u) | (ROL((uint32_t)VariableToConvert, 8) & 0x00FF00FFu))
#endif
 ...
 * @retval  The converted 32 bits value
 */
#define BE_CONVERT_W32(VariableToConvert) ((ROL(VariableToConvert, \
                                                24) & 0xFF00FF00) | (ROL(VariableToConvert, 8) & 0x00FF00FF))
#endif
...
#define W32_2_BUFFER(P_pOutputBuffer, P_index, P_word) *((uint32_t*)(P_pOutputBuffer + (P_index))) = LE_CONVERT_W32( \
                                                                                                                    P_word)
/**
 * @brief   This macro is used to write a 32bit integer into a byte buffer,
 ...
 */
#define BUFFER_2_W32(P_pInputBuffer, \
                     P_index)   (WORD8_TO_WORD32(P_pInputBuffer[(P_index)], P_pInputBuffer[(P_index) + 1], \
                                                 P_pInputBuffer[(P_index) + 2], \
                                                 P_pInputBuffer[(P_index) + 3]))
 ...
 * @note    This macro is for systems where unaligned reading is \b not allowed
 */
#define BUFFER_2_LEW32(P_pInputBuffer, \
                       P_index)   (WORD8_TO_WORD32((P_pInputBuffer)[(P_index) + 3], (P_pInputBuffer)[(P_index) + 2], \
                                                   (P_pInputBuffer)[(P_index) + 1], (P_pInputBuffer)[(P_index)]))
 ...
 */
#define W32_2_BUFFER(P_pOutputBuffer, P_index, P_word) WORD32_TO_WORD8((P_word), \
                                                                       P_pOutputBuffer[(P_index) + 3], \
                                                                       P_pOutputBuffer[(P_index) + 2], \
                                                                       P_pOutputBuffer[(P_index) + 1], \
                                                                       P_pOutputBuffer[(P_index)])
 ...
 * @note    This macro is for systems where unaligned writing is \b  not allowed
 */
#define LEW32_2_BUFFER(P_pOutputBuffer, P_index, P_word) WORD32_TO_WORD8((P_word), (P_pOutputBuffer)[(P_index)], \
                                                                         (P_pOutputBuffer)[(P_index) + 1], \
                                                                         (P_pOutputBuffer)[(P_index) + 2], \
                                                                         (P_pOutputBuffer)[(P_index) + 3])
